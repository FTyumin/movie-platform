...
<section class="cb-grain relative isolate overflow-hidden">
    ...
    <div class="relative mx-auto flex w-full max-w-[110rem] flex-col gap-8 px-6 pb-12 pt-28 sm:px-8 sm:pb-16 lg:flex-row lg:items-end">
        ...
        <div class="min-w-0">
            ...
            <div class="mt-5 flex flex-wrap items-center gap-x-3 gap-y-1 font-display text-[0.8rem] uppercase tracking-widest text-base-content/60">
                    ...
                    @if(!auth()->user()->lists->isEmpty())
                        <div class="relative" x-data="{ open: false }" @keydown.escape.window="open = false" @click.outside="open = false">
                            ...
                            <div x-cloak x-show="open" x-transition
                                 class="absolute left-0 top-full z-10 mt-2 w-52 overflow-hidden rounded-box border border-white/8 bg-base-200 py-1.5 shadow-xl">
                ...
                <div class="mt-6 flex items-center gap-3">
                    ...
                    <form action="{{ route('movies.destroy', $movie) }}" method="POST">
                        ...
                        <button type="submit" onclick="return confirm('Delete this movie?')"
                                class="inline-flex items-center gap-2 rounded-selector border border-error/30 px-5 py-3 font-display text-[0.72rem] font-semibold uppercase tracking-[0.12em] text-error/80 transition hover:border-error hover:bg-error/10 hover:text-error">
...
<section class="mx-auto w-full max-w-[110rem] px-6 pt-16 sm:px-8">
    <div class="grid gap-8 lg:grid-cols-3">
        <div class="lg:col-span-2">
                ...
                <div class="mt-6 flex flex-wrap gap-2">
                    @foreach($movie->genres as $genre)
                        <a href="{{ route('genres.show', $genre->id) }}"
                           class="rounded-selector border border-white/8 bg-base-200 px-4 py-1.5 font-display text-[0.7rem] uppercase tracking-widest text-base-content/60 transition hover:border-primary/40 hover:text-base-content">
                            {{ $genre->name }}
                        </a>
                    @endforeach
                </div>
            ...
            @if($movie->trailer_url)
                <div id="trailer" class="mt-12 scroll-mt-24">
                    ...
                    <div class="overflow-hidden rounded-box border border-white/6 bg-base-200">
                        <iframe
        ...
        <div class="h-max space-y-6 rounded-box border border-white/6 bg-base-200 p-6">
        ...
        @if($movie->actors->isNotEmpty())
            <div class="grid grid-cols-2 gap-5 sm:grid-cols-3 lg:grid-cols-5">
                @foreach($movie->actors->take(5) as $actor)
                    <a href="{{ route('people.show', $actor) }}" class="group relative block overflow-hidden rounded-box border border-white/6 bg-base-200">
...
<section class="mx-auto w-full max-w-[110rem] px-6 pt-16 sm:px-8">
    <div class="grid gap-8 lg:grid-cols-3">
        ...
        <div class="h-max rounded-box border border-white/6 bg-base-200 p-8 text-center">
        ...
        <div class="lg:col-span-2">
            ...
            @forelse($reviews->take(3) as $review)
                ...
                <article class="mb-4 rounded-box border border-white/6 bg-base-200 p-5 last:mb-0">
                    <div class="flex items-start justify-between gap-4">
                        <a href="{{ route('profile.show', $review->user) }}" class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="font-display text-[0.65rem] uppercase tracking-widest text-base-content/40">{{ $review->created_at->diffForHumans() }}</p>
                            </div>
                        </a>
            ...
            @empty
                <div class="rounded-box border border-white/6 bg-base-200 p-8 text-center">
